...
impl RawEmitter {
    pub fn emit_cascading_map(&mut self, ranges: &[Range<u32>], postfix : &str) -> bool {
        let mut map: [u8; 256] = [
        ...
        for high_byte in high_bytes {
            ...
            if codepoints.len() == 1 {
                ...
                arms.push(format!("{} => c.to_uint() == {:#04x}", high_byte, ch));
                continue;
            }
            ...
            arms.push(format!(
                "{} => (whitespace_map[(c.to_uint() & 0xff).reinterpret_as_int()] & {}) != (0x00 : Byte)",
                high_byte, bit_for_high_byte
            ));
            bit_for_high_byte <<= 1;
        }

        writeln!(&mut self.file, "let whitespace_map: Bytes = [{}];", fmt_list(map.iter()))
            .unwrap();
        self.bytes_used += 256;

        writeln!(&mut self.file, "pub fn lookup_{}(c : Char) -> Bool {{", postfix).unwrap();
        writeln!(&mut self.file, "    match c.to_uint() >> 8 {{").unwrap();
        for arm in arms {
