...
impl RawEmitter {
    pub fn emit_skiplist(&mut self, ranges: &[Range<u32>], postfix: &str) {
        ...
        writeln!(
            &mut self.file,
            "        // short_offset_runs_{}.last().unwrap().0 > @char.max_value",
            postfix
        )
        .unwrap();
        writeln!(&mut self.file, "        let mut i = 0;").unwrap();
        writeln!(&mut self.file, "        while i < short_offset_runs_{}.length() {{", postfix).unwrap();
        writeln!(
            &mut self.file,
            "            // short_offset_runs_{postfix}[i].start_index() < offsets_{postfix}.length()",
        )
        .unwrap();
