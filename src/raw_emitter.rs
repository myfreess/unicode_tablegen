...
impl RawEmitter {
    ...
    fn emit_bitset(&mut self, ranges: &[Range<u32>], postfix: &str) -> Result<(), String> {
        ...
        writeln!(
            &mut self.file,
            "let bitset_canonical_{postfix} : FixedArray[UInt64] = [{}];",
        ...
        writeln!(
            &mut self.file,
            "let bitset_mapping_{postfix} : FixedArray[(Byte, Byte)] = [{}];",
    ...
    fn emit_chunk_map(&mut self, zero_at: u8, compressed_words: &[u8], chunk_length: usize, postfix : &str) {
        ...
        writeln!(
            &mut self.file,
            "let bitset_chunks_map_{postfix} : Bytes = [{}];",
        ...
        writeln!(
            &mut self.file,
            "let bitset_index_chunks_{postfix} : FixedArray[Bytes] = [{}];",
