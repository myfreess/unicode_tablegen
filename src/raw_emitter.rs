...
impl RawEmitter {
    ...
    fn emit_bitset(&mut self, ranges: &[Range<u32>], postfix: &str) -> Result<(), String> {
        ...
        for length in 1..=64 {
            ...
            temp.emit_chunk_map(word_indices[&0], &compressed_words, length, postfix);
        ...
        writeln!(
            &mut self.file,
            "pub let bitset_canonical_{postfix} : FixedArray[UInt64] = [{}];",
        ...
        writeln!(
            &mut self.file,
            "pub let bitset_mapping_{postfix} : FixedArray[(Byte, Byte)] = [{}];",
        ...
        writeln!(&mut self.file, "pub fn lookup_{postfix}(c : Char) -> Bool {{").unwrap();
        ...
        writeln!(&mut self.file, "        bitset_chunks_map_{postfix},").unwrap();
        writeln!(&mut self.file, "        bitset_index_chunks_{postfix},").unwrap();
        writeln!(&mut self.file, "        bitset_canonical_{postfix},").unwrap();
        writeln!(&mut self.file, "        bitset_mapping_{postfix},").unwrap();
        writeln!(&mut self.file, "    )").unwrap();
        writeln!(&mut self.file, "}}").unwrap();
    ...
    fn emit_chunk_map(&mut self, zero_at: u8, compressed_words: &[u8], chunk_length: usize, postfix : &str) {
        ...
        writeln!(
            &mut self.file,
            "pub let bitset_chunks_map_{postfix} : Bytes = [{}];",
        ...
        writeln!(
            &mut self.file,
            "pub let bitset_index_chunks_{postfix} : FixedArray[Bytes] = [{}];",
