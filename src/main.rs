...
fn main() {
    ...
    table_file.push('\n');
    table_file.push_str(include_str!("range_search.mbt"));
    table_file.push('\n');
    ...
    modules.push(case_mapping::generate_case_mapping(&unicode_data));
...
fn version() -> String {
    ...
    out.push_str("pub let unicode_version : (Byte, Byte, Byte) = ");
...
fn generate_asserts(s: &mut String, property: &str, points: &[u32], truthy: bool) {
    for range in ranges_from_set(points) {
        if range.end == range.start + 1 {
            s.push_str(&format!("        // charcode {}\n", range.start));
            s.push_str(&format!(
                "        {}(is_{}({:?}))\n",
                if truthy { "assert_true" } else { "assert_false" },
                property.to_lowercase(),
                std::char::from_u32(range.start).unwrap(),
            ));
        } else {
            s.push_str(&format!("        for chn in {}U..<{}U {{\n", range.start, range.end));
            s.push_str(&format!(
                "            {}(is_{}(Int::unsafe_to_char(chn.reinterpret_as_int())), {})\n",
                if truthy { "ensure" } else { "ensure_not" },
                property.to_lowercase(),
                "\"charcode = \\{chn}\""
            ));
            s.push_str("        }\n");
