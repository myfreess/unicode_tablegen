...
fn generate_tables(case: &str, data: &BTreeMap<u32, (u32, u32, u32)>) -> String {
    ...
    write!(tables, "pub let {}case_table : FixedArray[(Char, UInt)] = [{}];", case, fmt_list(mappings))
        .unwrap();

    tables.push_str("\n\n");

    write!(tables, "pub let {}case_table_multi : FixedArray[(Char, Char, Char)] = [{}]", case, fmt_list(multis))
        .unwrap();
